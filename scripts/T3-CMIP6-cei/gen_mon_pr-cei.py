...
for file in os.listdir(in_path):
    if fnmatch.fnmatch(file, '*.nc'):
       #remane output file name [pr_day] to [pr-cei_mon]
       outfile="pr-cei_mon"+file[6:]
       ...
       odata = data.pr.where(data.pr <= 1.).resample(time='1M').count(dim='time')
       odata.name= 'dryday_1mm'
       odata.attrs['long_name'] = "Monthly dry days with precipitation less than 1mm (~0.04inch)"
       
       # dataarray to dataset (xarray)
       ods = odata.to_dataset()
       
       # dry day smaller than 0.01 inch (0.254mm)
       dryday=data.pr.where(data.pr <= .254).resample(time='1M').count(dim='time')
       dryday.attrs['long_name'] = "Monthly dry days with precipitation less than 0.254mm (0.01inch)"
       ods['dryday'] = dryday
       
       
       # wet day larger than 1 inch
       wetday_1inch = data.pr.where(data.pr >= 25.4).resample(time='1M').count(dim='time')
       wetday_1inch.attrs['long_name'] = "Monthly wet days with precipitation larger than 25.4mm (1inch)"
       ods['wetday_1inch'] = wetday_1inch
       
       # wet day larger than 2 inch
       wetday_2inch = data.pr.where(data.pr >= 50.8).resample(time='1M').count(dim='time')
       wetday_2inch.attrs['long_name'] = "Monthly wet days with precipitation larger than 50.8mm (2inch)"
       ods['wetday_2inch'] = wetday_2inch
       
       # wet day larger than 3 inch
       wetday_3inch = data.pr.where(data.pr >= 76.2).resample(time='1M').count(dim='time')
       wetday_3inch.attrs['long_name'] = "Monthly wet days with precipitation larger than 76.2mm (3inch)"
