...
in_path = "/scratch/04380/wenying/CMIP6/tasmin-cei/input"
out_path = "/scratch/04380/wenying/CMIP6/tasmin-cei/output"

for file in os.listdir(in_path):
    if fnmatch.fnmatch(file, '*.nc'):
       #remane output file name [tasmin_day] to [tasmin-cei_yr]
       outfile="tasmin-cei_yr"+file[10:]
       ...
       odata = data.tasmin.where(data.tasmin >= 305.372222).resample(time='1Y').count(dim='time')
       odata.name= 'tasmin_g90f'
       odata.attrs['long_name'] = "Yearly days with tasmin larger than 90F (~32.22C)"
       
       # dataarray to dataset (xarray)
       ods = odata.to_dataset()

       # Days w/tasmin above 80F (26.67C;299.816667K)
       tasmin_g80f=data.tasmin.where(data.tasmin  >= 299.816667).resample(time='1Y').count(dim='time')
       tasmin_g80f.attrs['long_name'] = "Yearly days (warm nights) with tasmin larger than 80F (~26.67C)"
       ods['tasmin_g80f'] =tasmin_g80f
       
       tasmin_l32f =data.tasmin.where(data.tasmin < 273.15).resample(time='1Y').count(dim='time')
       tasmin_l32f.attrs['long_name'] = "Yearly frost days (cold nights) with tasmin smaller than freezing point"
